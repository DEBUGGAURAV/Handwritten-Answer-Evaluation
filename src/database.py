...
def register_user(username: str, password: str, email: str) -> dict:
    ...
    db.users.insert_one({
        ...
        "otp_expiry": datetime.datetime.utcnow() + datetime.timedelta(minutes=OTP_VALIDITY_MINUTES),
...
def verify_otp(username: str, otp: str) -> dict:
    ...
    if datetime.datetime.utcnow() > user.get("otp_expiry", datetime.datetime.min):
...
def resend_otp(username: str) -> dict:
    ...
    log_event("otp_resent", username, {"email_sent": email_sent})
    ...
    return {
        ...
        "message": (
            ...
            if email_sent
            else "Failed to send OTP email. Check the email server configuration."
        ),
    }
...
def save_grading_result(
    ...
    doc = {
        ...
        "max_marks": max_marks,
        "percentage": (round(100 * marks_obtained / max_marks, 2) if max_marks else 0),
        "details": details,
        "created_at": datetime.datetime.utcnow(),
    }
...
def get_results_for_user(username: str, limit: int = 200) -> List[dict]:
    db = get_db()

    return list(db.results.find({"username": username}).sort("created_at", -1).limit(limit))
...
def save_uploaded_file_metadata(
    ...
    return str(db.uploaded_files.insert_one(doc).inserted_id)


def log_event(event_type: str, username: str, data: dict):
    db = get_db()
...
def get_logs(limit: int = 200) -> List[dict]:
    ...
    return list(db.logs.find().sort("timestamp", -1).limit(limit))
